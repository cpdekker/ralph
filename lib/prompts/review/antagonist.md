...
2. **REQUIRED: Use the Task tool to review items in parallel** — you MUST launch one Task per item:
   ...
   - Each Task reads the relevant source files closely and independently
   - Each Task hunts for AI code smells (see detailed checklist below)
   - Each Task evaluates whether the code looks like a human wrote it with intent
