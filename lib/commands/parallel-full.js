...
async function run(spec, opts = {}) {
  ...
  console.log(`\n${c('magenta', 'PARALLEL FULL MODE')}`);
  console.log(`${c('cyan', `Spec: ${spec}`)}`);
  console.log(`${c('cyan', `Sub-specs: ${totalSpecs} (${completedSpecs} complete)`)}`);
  console.log(`${c('cyan', `Max parallel: ${parallel}`)}\n`);
  ...
  if (completedSpecs === totalSpecs) {
    console.log(c('green', '  All sub-specs completed successfully!'));
    console.log(c('cyan', '  Running master completion check...'));
    // The master completion check runs as part of the normal full mode
    // The user can run `ralph full <spec>` to do the final check
    console.log(c('cyan', `  Run "ralph full ${spec}" for master completion check`));
  } else {
    console.log(c('yellow', `  ${completedSpecs}/${totalSpecs} sub-specs completed`));
    const failed = Object.entries(graph).filter(([_, s]) => s.status === 'failed' || s.status === 'merge_conflict');
    if (failed.length > 0) {
      console.log(c('red', '  Failed sub-specs:'));
      for (const [name, s] of failed) {
        console.log(c('red', `    - ${name} (${s.status})`));
      }
    }
  }
