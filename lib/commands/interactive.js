...
async function run() {
  ...
  while (!mode) {
    ...
    if (!mode) console.log(c('red', '  Invalid selection. Enter 1-8 or mode name.'));
  }

  const iterLabel = mode === 'full' ? 'max cycles' : 'iterations';
  const defaultN = defaultIterations[mode];
  const iterInput = await question(`  Number of ${iterLabel} (default: ${defaultN}): `);
  const iterations = iterInput.trim() || defaultN;

  const verboseInput = await question('  Verbose output? (default: No) [y/N]: ');
  const verbose = verboseInput.trim().toLowerCase() === 'y';

  let background = false;
  if (!['debug', 'decompose', 'spec'].includes(mode)) {
    const bgDefault = mode === 'full';
