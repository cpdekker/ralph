...
function runForeground(spec, mode, iterations, verbose) {
  const root = repoDir();
...
function runBackground(spec, mode, iterations, verbose) {
  const root = repoDir();
  const imageName = getImageName();
...
async function promptCustomization(mode, defaults) {
  ...
  rl.close();
  return { iterations, verbose, background };
}
...
async function run(spec, mode, opts = {}) {
  ...
  // Validate spec exists (unless in spec creation mode)
  ...
  if (!skipPrompts) {
    const customized = await promptCustomization(mode, { iterations, verbose, background });
  ...
  if (background) {
    runBackground(spec, mode, iterations, verbose);
  } else {
    runForeground(spec, mode, iterations, verbose);
  }
}
...
function getDefaultIterations(mode) {
  const defaults = {
    ...
    debug: 1, full: 10, decompose: 1, spec: 8,
  };
