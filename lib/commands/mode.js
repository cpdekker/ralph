...
const { getImageName, ensureImage, isDockerAvailable } = require('../utils/docker');
...
function runLocal(spec, mode, iterations, verbose, insights, insightsGithub) {
  const root = repoDir();
  const loopScript = path.join(libDir, 'scripts', 'loop.sh');

  const bashArgs = [toBashPath(loopScript), spec, mode, String(iterations)];
  if (verbose) bashArgs.push('--verbose');

  // Build env: inherit process env + .ralph/.env + RALPH_LIB_DIR
  ...
  if (insights) env.RALPH_INSIGHTS = 'true';
  if (insightsGithub) env.RALPH_INSIGHTS_GITHUB = 'true';

  const loopProcess = spawn('bash', bashArgs, {
    stdio: 'inherit',
    cwd: root,
    env,
  });
  ...
  loopProcess.on('error', (err) => {
    error(`Failed to start loop: ${err.message}`);
    if (err.code === 'ENOENT') {
      info('bash not found. Install Git for Windows (includes Git Bash) or use Docker mode.');
    }
    process.exit(1);
  });
}
...
async function run(spec, mode, opts = {}) {
  ...
  if (!local) info(`Background:  ${background}`);
  info(`Local:       ${local}`);
  ...
  if (mode === 'spec') {
    ...
    if (!local) prepareSpecBranch(spec);
  }

  setupWindowsSignalHandler();

  if (local) {
    // Local mode: run loop.sh directly via bash, no Docker needed
    runLocal(spec, mode, iterations, verbose, insights, insightsGithub);
  } else {
    ensureImage();
    if (background) {
