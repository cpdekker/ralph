...
const { c, error, warn, info, header, separator, hint, startupBanner, showPostRunHints } = require('../utils/colors');
const {
...
const { createInteractiveLogViewer } = require('../utils/interactive-logs');
...
async function researchGatherWizard(specName) {
  const seedPath = path.join(ralphDir(), 'research_seed.md');
  const referencesDir = path.join(ralphDir(), 'references');
  ...
  console.log(`\n${c('magenta', 'Research Gather Wizard')}`);
  console.log(c('dim', '─────────────────────────────────\n'));
  ...
  if (fs.existsSync(referencesDir)) {
    ...
    if (refFiles.length > 0) {
      ...
      console.log(c('dim', '   These will be reviewed and built upon during research.\n'));
    }
  } else {
    fs.mkdirSync(referencesDir, { recursive: true });
    console.log(c('dim', '  Created .ralph/references/ directory for research output.\n'));
  }

  // Check for existing research_seed.md
  if (fs.existsSync(seedPath)) {
    console.log(c('yellow', '  Found existing research_seed.md'));
    const reuse = await question('  Skip wizard and continue research? [Y/n]: ');
    if (reuse.trim().toLowerCase() !== 'n' && reuse.trim().toLowerCase() !== 'no') {
      console.log(c('green', '  ✓ Reusing existing research_seed.md\n'));
      rl.close();
      return;
    }
    console.log('');
  ...
  console.log(c('dim', '  What feature, change, or problem do you want to research? (1-3 sentences)'));
  const topicLines = [];
  while (true) {
    const line = await question('  > ');
    if (line.trim() === '') {
  ...
  console.log(c('dim', '  What specific questions do you want answered? (optional)'));
  const questions = await readMultiline('Questions:');
  console.log('');

  // Step 3: Areas of focus
  console.log(c('cyan', '  [3/4] Areas of Focus'));
  console.log(c('dim', '  Any specific areas to focus on? (e.g., "security implications", "database performance") (optional)'));
  const focuses = await readMultiline('Focus areas:');
  console.log('');

  // Step 4: Known context
  console.log(c('cyan', '  [4/4] Known Context'));
  console.log(c('dim', '  Any context you already have that would help the research? (optional)'));
  const context = await readMultiline('Context:');

  rl.close();

  // Build research_seed.md
  ...
  fs.writeFileSync(seedPath, seed);
  console.log(`\n${c('green', '✓ Created .ralph/research_seed.md')}\n`);
}

function prepareSpecBranch(spec, seedFile = '.ralph/spec_seed.md') {
  const root = repoDir();
  ...
  try {
    ...
    if (currentBranch === targetBranch) {
      // Already on the target branch — just commit and push the seed
      execSync(`git add ${seedFile}`, { cwd: root, stdio: 'ignore' });
      execSync('git commit -m "ralph: add seed for ' + spec + '"', { cwd: root, stdio: 'ignore' });
    } else {
      // Check if target branch already exists locally
      let branchExists = false;
      try {
        execSync(`git show-ref --verify --quiet refs/heads/${targetBranch}`, { cwd: root, stdio: 'ignore' });
        branchExists = true;
      } catch {}

      if (branchExists) {
        execSync(`git checkout ${targetBranch}`, { cwd: root, stdio: 'ignore' });
      } else {
        // Check if it exists on remote
        let remoteExists = false;
        try {
          execSync(`git ls-remote --exit-code --heads origin ${targetBranch}`, { cwd: root, stdio: 'ignore' });
          remoteExists = true;
        } catch {}

        if (remoteExists) {
          execSync(`git checkout -b ${targetBranch} origin/${targetBranch}`, { cwd: root, stdio: 'ignore' });
        } else {
          execSync(`git checkout -b ${targetBranch}`, { cwd: root, stdio: 'ignore' });
        }
      }
    ...
    // Push the branch so background clones can access the seed
    try {
      execSync(`git push -u origin ${targetBranch}`, { cwd: root, stdio: 'ignore' });
    } catch {
      execSync(`git push origin ${targetBranch}`, { cwd: root, stdio: 'ignore' });
    }

    console.log(c('green', `✓ Committed seed to branch ${targetBranch} and pushed`));
  } catch (err) {
    console.log(c('yellow', `Warning: Could not prepare spec branch: ${err.message}`));
    console.log(c('yellow', '   Continuing — the loop will handle branch creation.'));
  }
}
...
function runBackground(spec, mode, iterations, verbose, insights, insightsGithub) {
  ...
  try {
    ...
    // Remove process-level SIGINT handlers — the interactive viewer handles Ctrl+C
    ...
    viewer = createInteractiveLogViewer({
      containerName,
      targetBranch,
      spec,
      mode,
...
async function run(spec, mode, opts = {}) {
  ...
  if (!spec) {
    ...
    if (mode === 'spec' || mode === 'research') {
      // Spec/research mode: need a name
      console.error(c('red', `\n  Error: Please provide a name. Usage: ralph ${mode} <name>\n`));
      process.exit(1);
    }
    if (specs.length === 0) {
  ...
  // Validate spec exists (unless in spec creation, research, or insights mode)
  if (mode !== 'spec' && mode !== 'research' && mode !== 'insights' && !validateSpec(spec)) {
  ...
  startupBanner({ cwd: repoDir(), spec, mode });
  ...
  if (insights) info(`Insights:    enabled${insightsGithub ? ' (+ GitHub issues)' : ''}`);
  console.log('');

  ensureImage();
  checkEnvFile();

  // Spec mode: run gather wizard first, then commit seed to branch
  if (mode === 'spec') {
    await specGatherWizard(spec);
    prepareSpecBranch(spec);
  }

  // Research mode: run research gather wizard, then commit seed to branch
  if (mode === 'research') {
    await researchGatherWizard(spec);
    prepareSpecBranch(spec, '.ralph/research_seed.md');
  }
...
function getDefaultIterations(mode) {
  const defaults = {
    ...
    debug: 1, full: 10, decompose: 1, spec: 8, research: 10, insights: 1,
  };
