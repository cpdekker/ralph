...
function prepareSpecBranch(spec) {
  const root = repoDir();
  ...
  try {
    ...
    if (currentBranch === targetBranch) {
      ...
      execSync('git add .ralph/spec_seed.md', { cwd: root, stdio: 'ignore' });
      execSync('git commit -m "ralph: add spec seed for ' + spec + '"', { cwd: root, stdio: 'ignore' });
    } else {
    ...
    console.log(c('green', `✓ Committed spec_seed.md to branch ${targetBranch} and pushed`));
  } catch (err) {
