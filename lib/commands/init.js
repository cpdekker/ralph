...
const { c, success, warn, error, info, step, createSpinner, startupBanner, header } = require('../utils/colors');
...
function commitInitBranch() {
  const root = repoDir();
  const branch = 'ralph/init';
  ...
  try {
    ...
    // Stage all ralph files and .gitignore (but not .env which is gitignored)
    execSync('git add .ralph/ .gitignore', { cwd: root, stdio: 'ignore' });
    execSync('git commit -m "ralph: initialize .ralph directory"', { cwd: root, stdio: 'ignore' });
    ...
    success(`Committed and pushed to branch ${branch}`);
  } catch (err) {
    warn(`Could not create init branch: ${err.message}`);
    info('You can commit the .ralph directory manually.');
  }
}
...
async function run() {
  ...
  try {
    ...
    step(7, totalSteps, 'Creating ralph/init branch');
    commitInitBranch();
