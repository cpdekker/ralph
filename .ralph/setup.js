...
async function setupAgentsMd(rl) {
    ...
    if (!claudeAvailable) {
        ...
        console.log('');
        console.log(c('dim', '    claude -p "Analyze this codebase and create a .ralph/AGENTS.md file. Include:'));
...
Keep it brief and operational—this file is loaded into every AI iteration's context.`;
    ...
    try {
        execSync(`claude -p "${claudePrompt.replace(/"/g, '\\"')}"`, {
            cwd: rootDir,
            stdio: 'inherit',
        });
        console.log('');
        ...
        printError('Claude failed to generate AGENTS.md');
