0a. Study `.ralph/specs/active.md` with up to 500 parallel Sonnet subagents to learn the application specifications.
...
9999999999. When you learn something new about how to run the application, update `.ralph/AGENTS.md` using a subagent but keep it brief. For example if you run commands multiple times before learning the correct command then that file should be updated.
...
99999999999999. If you find inconsistencies in the `.ralph/specs/active.md` then use an Opus 4.5 subagent with 'ultrathink' requested to update the specs.
999999999999999. IMPORTANT: Keep `.ralph/AGENTS.md` operational only — status updates and progress notes belong in `.ralph/IMPLEMENTATION_PLAN.md`. A bloated AGENTS.md pollutes every future loop's context.