...
# Run loop directly
docker run -it --rm --env-file .ralph/.env -v "${PWD}:/workspace" -w /workspace ralph-wiggum ./.ralph/scripts/loop.sh plan 2
```
...
| Command | Description |
|---------|-------------|
| `./.ralph/scripts/loop.sh plan 2` | Research/planning only, 2 iterations |
| `./.ralph/scripts/loop.sh 5` | Build mode, 5 iterations |
| `./.ralph/scripts/loop.sh` | Build mode, 10 iterations (default) |
...
**Image not found:** Rebuild with `docker build -t ralph-wiggum -f .ralph/docker/Dockerfile .`

**Permission denied on scripts:** Run `chmod +x .ralph/scripts/*.sh .ralph/docker/entrypoint.sh` inside container
